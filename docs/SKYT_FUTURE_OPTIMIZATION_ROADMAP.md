# Ai Multi Agent 后续优化路线图

本文面向后续维护、继续生成代码和 vibe coding，不是宣传文案。重点记录 Ai Multi Agent 继续演进时应遵守的数据结构、Token 统计、执行过程可视化和本地 GPU + API 协作边界。

## 1. 当前定位

Ai Multi Agent 是比赛展示型 AI 工作台，核心价值不是堆功能数量，而是让评委和用户看到一个能对话、能绑定工作区、能执行任务、能记录运行过程、能回放 I/O Payload 的本地 AI Agent 系统。

后续优化要优先服务三个目标：

- 对话与执行过程可信：用户知道 Ai Multi Agent 现在在做什么。
...
Ai Multi Agent 不应照搬它的 Vue/Java Gateway 架构，也不应把比赛工作台改造成另一个项目形态。
